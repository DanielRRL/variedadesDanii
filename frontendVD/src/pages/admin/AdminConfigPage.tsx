...
import { Settings, Store, Truck, Scale, Gamepad2, Bell, Shield, AlertTriangle } from 'lucide-react';
import '../../css/AdminConfigPage.css';

interface ConfigItem { label: string; value: string; description: string; }

function ConfigSection({ icon: Icon, title, items }: { icon: React.ElementType; title: string; items: ConfigItem[] }) {
  return (
    <div className="admin-config__card">
      <div className="admin-config__card-header">
        <Icon size={16} className="admin-config__card-icon" />
        <h2 className="admin-config__card-title">{title}</h2>
      </div>
      <div className="admin-config__items">
        {items.map(item => (
          <div key={item.label} className="admin-config__item">
            <div className="admin-config__item-info">
              <p className="admin-config__item-label">{item.label}</p>
              <p className="admin-config__item-desc">{item.description}</p>
            </div>
            <span className="admin-config__item-value">{item.value}</span>
          </div>
        ))}
      </div>
    </div>
  ...
  return (
    <div className="admin-config">
      <div className="admin-config__header">
        <div className="admin-config__header-info">
          <h1 className="admin-config__title">Configuración</h1>
          <p className="admin-config__subtitle">Parámetros del sistema y reglas de negocio</p>
        </div>
        <Settings size={24} className="admin-config__header-icon" />
      </div>

      <div className="admin-config__warning">
        <AlertTriangle size={20} className="admin-config__warning-icon" />
        <div>
          <h3 className="admin-config__warning-title">Configuración de referencia</h3>
          <p className="admin-config__warning-text">
      ...
      <div className="admin-config__grid">
        <ConfigSection icon={Store} title="Tienda" items={[
        ...
        <ConfigSection icon={Truck} title="Envíos" items={[
        ...
        <ConfigSection icon={Scale} title="Sistema de Gramos" items={[
        ...
        <ConfigSection icon={Gamepad2} title="Juegos y Fichas" items={[
        ...
        <ConfigSection icon={Bell} title="Notificaciones" items={[
        ...
        <ConfigSection icon={Shield} title="Seguridad" items={[
