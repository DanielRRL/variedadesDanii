...
import {
  Eye, EyeOff, User, Phone, Mail, Lock, Check, X,
...
function Stepper({ step }: { step: number }) {
  const labels = ['Datos personales', 'Contraseña segura', 'Verificar email'];
      ...
      {labels.map((label, i) => {
        ...
        return (
          <div key={n} className="flex items-center">
            {i > 0 && (
              <div className={`w-10 h-0.5 mx-1 ${done ? 'bg-green-500' : 'bg-gray-200'}`} />
            )}
            <div
            ...
            <span className={`ml-2 text-xs hidden sm:inline ${active ? 'text-text-primary font-medium' : 'text-muted'}`}>
              {label}
            </span>
          </div>
        );
      ...
      {step === 1 && (
        <form onSubmit={handleStep1} className="flex flex-col gap-4">
          ...
          <div className="flex flex-col gap-1.5">
            ...
            <p className="text-xs text-muted">Te enviaremos un código de verificación a este correo</p>
          </div>
            ...
            {showReferral && (
              <>
                <input
                  ...
                  className={`border rounded-xl px-3 py-3 text-sm focus:outline-none focus:ring-2 focus:ring-brand-pink/40 transition-colors uppercase tracking-widest ${
                ...
                />
                {referralValid === true && (
                  <span className="text-xs text-green-600">Código válido. Ambos recibirán puntos de bonificación.</span>
                )}
                {referralValid === false && (
          ...
          <button
            type="submit"
            className="w-full bg-brand-pink hover:bg-pink-700 text-white font-heading font-semibold py-3 rounded-full transition-colors text-sm mt-1"
          >
            Continuar
          </button>
          ...
          <div className="flex items-center gap-3 my-1">
            ...
            <span className="text-xs text-muted">ó registrate con</span>
            <div className="flex-1 h-px bg-border" />
          </div>
        ...
        <form onSubmit={handleStep2} className="flex flex-col gap-4">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
            <div className="flex flex-col gap-1.5">
              ...
              {password.length > 0 && (
                <div className="flex flex-col gap-1 mt-1">
                  ...
                  <span className={`text-xs ${score >= 4 ? 'text-green-600' : 'text-muted'}`}>
                    {score >= 4 ? <span className="flex items-center gap-1"><Check size={12} /> {STRENGTH_LABELS[score]}</span> : STRENGTH_LABELS[score]}
                  </span>
                </div>
              )}
            </div>
            ...
            <div className="flex flex-col gap-1.5">
              ...
              {confirmPassword && (
                <span className={`text-xs flex items-center gap-1 ${passwordMatch ? 'text-green-600' : 'text-red-500'}`}>
          ...
          <label className="flex items-start gap-2.5 cursor-pointer">
            ...
            <span className="text-xs text-muted leading-relaxed">
              Acepto los{' '}
              <a href="#" className="text-brand-pink underline">términos y condiciones</a>
              {' '}y la{' '}
              <a href="#" className="text-brand-pink underline">política de privacidad</a>
