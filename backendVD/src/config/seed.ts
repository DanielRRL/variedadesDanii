 ...
 * Si el admin existe, solo garantiza que este activo y verificado.
 * NUNCA sobrescribe la contrasena de un admin existente — si se necesita
 * cambiar la contrasena del admin, usar la funcionalidad de reset-password.
 */
export async function seedAdminUser(): Promise<void> {
  try {
    ...
    if (existing) {
      // Admin ya existe: solo asegurar que este activo y verificado.
      // No modificar su contrasena (pudo haber sido cambiada manualmente).
      if (!existing.active || !existing.emailVerified || existing.role !== "ADMIN") {
        ...
        logger.info("Admin user updated (active/verified/role only)");
      }
      logger.info(`Admin user ready: ${existing.email} (${existing.id})`);
      return;
    }

    // Admin no existe: crear con los valores del .env
    ...
    logger.info(`Admin user created: ${admin.email} (${admin.id})`);
  } catch (error) {
